...
def page():
    ...
    return ("<!doctype html><html lang=de><head><meta charset=utf-8>"
            ...
            "body{font-family:system-ui,sans-serif;max-width:760px;margin:0 auto;padding:1em;"
            "background:#0b1020;color:#e6e9f0}h2{margin:.2em 0}ul{padding:0}"
            "li{list-style:none;background:#161c33;border-radius:10px;padding:.6em .8em;margin:.55em 0}"
            "li.marked{opacity:.4}li.marked .t{text-decoration:line-through}"
            ".hd{display:flex;align-items:center;gap:10px}.t{font-weight:600}"
            ".s{color:#9aa3bd;font-size:.85em}.dl{margin-left:auto;color:#7fd1ff;text-decoration:none;font-size:1.2em}"
            ".del{background:#5a1d1d;color:#ffd9d9;border:none;border-radius:7px;width:34px;height:30px;font-size:15px}"
            ".del:active{background:#7a2626}li.marked .del{background:#2a3550;color:#9aa3bd}"
            ".res{display:block;width:100%;margin-top:.5em;background:#1e6e44;color:#eafff2;"
            "border:none;border-radius:8px;padding:.7em;font-size:16px;font-weight:600}.res:active{background:#15512f}"
            ".cb{flex:0 0 auto;width:22px;height:22px;border:2px solid #3a4a63;border-radius:5px;cursor:pointer}"
            "li.sel .cb{background:#7fd1ff;border-color:#7fd1ff}li.sel{outline:2px solid #7fd1ff}"
            ".bar{position:sticky;top:0;z-index:5;background:#0b1020;padding:.5em 0;margin-bottom:.3em;"
            "display:none;align-items:center;gap:8px;flex-wrap:wrap}.bar.on{display:flex}"
            ".bar .cnt{font-weight:600}.bar button{background:#1d3a5a;color:#cfe6ff;border:none;"
            ...
            "details{margin-top:1.2em}summary{cursor:pointer}audio{width:100%;margin-top:.45em}"
            "</style></head><body><h2>&#9875; VHF-Aufnahmen</h2>"
            "<p class=s>Antippen zum Abh&ouml;ren &middot; &#11015; Download &middot; &#10005; einzeln ausblenden &middot; "
            ...
            "<ul id=list>" + "".join(rows) + "</ul>" + noise_section +
            "<script>"
